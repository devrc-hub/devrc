...
use crate::{
    colors,
    media_type::{serialize_media_type, MediaType},
};

use deno_core::{resolve_url, serde::Serialize, ModuleSpecifier};
use std::{collections::HashSet, fmt, iter::Iterator, path::PathBuf};
...
impl ModuleGraphInfoDep {
    fn write_info<S: AsRef<str> + fmt::Display + Clone>(
        ...
        match (maybe_code, maybe_type) {
            ...
            (Some(code), None) => code.write_info(f, prefix, last, false, modules, seen),
            (None, Some(types)) => types.write_info(f, prefix, last, true, modules, seen),
            _ => Ok(()),
        }
    }
}
...
impl ModuleGraphInfoMod {
    fn write_info<S: AsRef<str> + fmt::Display>(
        ...
        let (size, specifier) = if self.error.is_some() {
            ...
            (
                colors::gray(format!(" ({})", human_size(self.size.unwrap_or(0) as f64)))
                    .to_string(),
                name,
            )
        };
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(err) = &root.error {
            ...
            writeln!(
                ...
                "\n{} {}",
                self.root,
                colors::gray(format!("({})", human_size(root.size.unwrap_or(0) as f64)))
            )?;
...
mod test {
    use super::*;
    use deno_core::{resolve_url, serde_json::json};
