...
use crate::{
    ast,
    ast::{parse, transpile_module, BundleHook, Location, ParsedModule},
    checksum, colors,
    ...
    specifier_handler::{
        CachedModule, Dependency, DependencyMap, Emit, FetchFuture, SpecifierHandler,
    },
    tsc,
    tsc_config::{IgnoredCompilerOptions, TsConfig},
    version,
};
use deno_core::error::AnyError;

use deno_core::{
    error::{anyhow, custom_error, get_custom_error_class, Context},
    futures::stream::{FuturesUnordered, StreamExt},
    resolve_url_or_path,
    serde::{Deserialize, Deserializer, Serialize, Serializer},
    serde_json::{json, Value},
    ModuleResolutionError, ModuleSource, ModuleSpecifier,
};
use regex::Regex;
use std::{
...
impl swc_bundler::Load for BundleLoader<'_> {
    fn load(&self, file: &swc_common::FileName) -> Result<swc_bundler::ModuleData, AnyError> {
        match file {
            swc_common::FileName::Custom(filename) => {
                ...
                if let Some(src) = self.graph.get_source(&specifier) {
                    ...
                    Err(GraphError::MissingDependency(specifier, "<bundle>".to_string()).into())
                }
            }
...
impl Module {
    ...
    pub fn parse(&mut self) -> Result<ParsedModule, AnyError> {
        let parsed_module = parse(self.specifier.as_str(), &self.source, &self.media_type)?;
            ...
            if let Some(ts_reference) = parse_ts_reference(&comment.text) {
                ...
                match ts_reference {
                    TypeScriptReference::Path(import) => {
                        let specifier = self.resolve_import(&import, Some(location.clone()))?;
                        let dep = self
                            .dependencies
                            .entry(import)
        ...
        for desc in dependencies
            .iter()
            .filter(|desc| desc.kind != swc_ecmascript::dep_graph::DependencyKind::Require)
        {
            ...
            let maybe_specifier = match self.resolve_import(&desc.specifier, Some(location.clone()))
            {
                Ok(specifier) => Some(specifier),
                Err(any_error) => match any_error.downcast_ref::<ModuleResolutionError>() {
    ...
    fn resolve_import(
        ...
        if referrer_scheme == "https" && specifier_scheme == "http" {
            return Err(GraphError::InvalidDowngrade(specifier.clone(), location).into());
        }
        ...
        if (referrer_scheme == "https" || referrer_scheme == "http")
            ...
            && !remapped_import
        {
            return Err(GraphError::InvalidLocalImport(specifier.clone(), location).into());
        }
...
pub struct ResultInfo {
    ...
    pub loadable_modules: HashMap<ModuleSpecifier, Result<ModuleSource, AnyError>>,
...
impl Graph {
    ...
    pub fn check(self, options: CheckOptions) -> Result<ResultInfo, AnyError> {
        ...
        let maybe_ignored_options = config.merge_tsconfig(options.maybe_config_path)?;
        ...
        if !self.needs_emit(&config)
            || (self.is_emit_valid(&config) && (!options.reload || self.roots_dynamic))
        {
        ...
        if response.diagnostics.is_empty() {
            if !response.emitted_files.is_empty() {
                ...
                for (specifier, code) in codes.iter() {
                    if let ModuleSlot::Module(module) = graph.get_module_mut(specifier).unwrap() {
    ...
    pub fn emit(
        ...
        config.merge(&opts);
        let maybe_ignored_options = if let Some(user_options) = &options.maybe_user_config {
        ...
        if options.check {
            ...
            match options.bundle_type {
                BundleType::Esm | BundleType::Iife => {
                    ...
                    let s = graph.emit_bundle(specifier, &config.into(), &options.bundle_type)?;
            ...
            match options.bundle_type {
                BundleType::Esm | BundleType::Iife => {
                    ...
                    let s = self.emit_bundle(specifier, &config.into(), &options.bundle_type)?;
                    emit_count += 1;
                    emitted_files.insert("deno:///bundle.js".to_string(), s);
                }
                BundleType::None => {
                    ...
                    for (_, module_slot) in self.modules.iter_mut() {
                        if let ModuleSlot::Module(module) = module_slot {
                            ...
                            if let Some(map) = maybe_map {
                                emitted_files.insert(format!("{}.js.map", module.specifier), map);
                            }
                        }
                    }
                    self.flush()?;
    ...
    pub fn get_loadable_modules(&self) -> HashMap<ModuleSpecifier, Result<ModuleSource, AnyError>> {
        let mut loadable_modules: HashMap<ModuleSpecifier, Result<ModuleSource, AnyError>> =
            self.modules.iter().map(to_module_result).collect();
        for (specifier, _) in self.redirects.iter() {
            if let Some(module_slot) = self.modules.get(self.resolve_specifier(specifier)) {
    ...
    pub fn get_media_type(&self, specifier: &ModuleSpecifier) -> Option<MediaType> {
    ...
    fn get_module_mut(&mut self, specifier: &ModuleSpecifier) -> Option<&mut ModuleSlot> {
            ...
            .filter_map(|(sp, sl)| match sl {
                ModuleSlot::Module(module) => {
                    let mut dependencies: Vec<info::ModuleGraphInfoDep> = module
                        ...
                        .collect();
                    dependencies.sort();
                    let (emit, map) = if let Some((emit, maybe_map)) = &module.maybe_emit_path {
    ...
    pub fn resolve(
        ...
        if !module.dependencies.contains_key(specifier) {
            return Err(
                GraphError::MissingDependency(referrer.to_owned(), specifier.to_owned()).into(),
            );
        }
        ...
        let dep_module =
            if let ModuleSlot::Module(dep_module) = self.get_module(&resolved_specifier) {
                dep_module
            } else {
    ...
    fn resolve_specifier<'a>(&'a self, specifier: &'a ModuleSpecifier) -> &'a ModuleSpecifier {
        let mut s = specifier;
        ...
        while let Some(redirect) = self.redirects.get(s) {
            if !seen.insert(redirect.clone()) {
                eprintln!(
                    "An infinite loop of module redirections detected.\n  Original specifier: {}",
                    specifier
                );
                break;
            }
    ...
    ///   transpiled.
    ///
    pub fn transpile(&mut self, options: TranspileOptions) -> Result<ResultInfo, AnyError> {
        let start = Instant::now();
        ...
        let maybe_ignored_options = ts_config.merge_tsconfig(options.maybe_config_path)?;
    ...
    fn validate(&self) -> Result<(), AnyError> {
            ...
            F: Fn(&ModuleSpecifier) -> ModuleSlot,
        {
            ...
            match get_module(specifier) {
                ...
                ModuleSlot::None => Err(custom_error(
                    "NotFound",
                    format!(
                        "The specifier \"{}\" is unexpectedly not in the module graph.",
                        specifier
                    ),
                )),
                ModuleSlot::Pending => Err(custom_error(
                    "InvalidState",
                    format!(
                        "The specifier \"{}\" is in an unexpected state in the module graph.",
                        specifier
                    ),
                )),
            }
...
impl GraphBuilder {
    ...
    fn fetch(
        ...
        if !self.graph.modules.contains_key(&specifier) {
            ...
            let future = handler.fetch(specifier.clone(), maybe_referrer.clone(), is_dynamic);
    ...
    fn visit(&mut self, cached_module: CachedModule, is_root: bool) -> Result<(), AnyError> {
        let specifier = cached_module.specifier.clone();
        let requested_specifier = cached_module.requested_specifier.clone();
        let mut module = Module::new(cached_module, is_root, self.maybe_import_map.clone());
        match module.media_type {
            MediaType::Json
            | MediaType::SourceMap
            ...
            | MediaType::Unknown => {
                return Err(
                    GraphError::UnsupportedImportType(module.specifier, module.media_type).into(),
                );
            }
            _ => (),
        }
...
pub mod tests {
    ...
    impl MockSpecifierHandler {
        fn get_cache(
            ...
            let source =
                fs::read_to_string(&source_path).map_err(|err| (specifier.clone(), err.into()))?;
    ...
    impl SpecifierHandler for MockSpecifierHandler {
        ...
        fn set_cache(&mut self, specifier: &ModuleSpecifier, emit: &Emit) -> Result<(), AnyError> {
    ...
    async fn setup(specifier: ModuleSpecifier) -> (Graph, Arc<Mutex<MockSpecifierHandler>>) {
    ...
    async fn setup_memory(specifier: ModuleSpecifier, sources: HashMap<&str, &str>) -> Graph {
    ...
    async fn test_graph_check_emit() {
        let specifier =
            resolve_url_or_path("file:///tests/main.ts").expect("could not resolve module");
    ...
    async fn fix_graph_check_emit_diagnostics() {
        let specifier =
            resolve_url_or_path("file:///tests/diag.ts").expect("could not resolve module");
        let (graph, handler) = setup(specifier).await;
        let result_info = graph
            .check(CheckOptions {
                debug: false,
    ...
    async fn fix_graph_check_mjs_root() {
        let specifier =
            resolve_url_or_path("file:///tests/a.mjs").expect("could not resolve module");
        let (graph, handler) = setup(specifier).await;
        let result_info = graph
            .check(CheckOptions {
                debug: false,
    ...
    async fn fix_graph_check_types_root() {
        let specifier =
            resolve_url_or_path("file:///typesref.js").expect("could not resolve module");
        let (graph, _) = setup(specifier).await;
        let result_info = graph
            .check(CheckOptions {
                debug: false,
    ...
    async fn test_graph_import_json() {
        let specifier =
            resolve_url_or_path("file:///tests/importjson.ts").expect("could not resolve module");
        let c = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").unwrap());
        let fixtures = c.join("tests/module_graph");
        let handler = Arc::new(Mutex::new(MockSpecifierHandler {
            fixtures,
    ...
    "#,
        ...
        let specifier =
            resolve_url_or_path("file:///tests/importremap.ts").expect("could not resolve module");
    ...
    async fn test_graph_with_lockfile() {
        ...
        let lockfile = Lockfile::new(lockfile_path, false).expect("could not load lockfile");
