...
use crate::{media_type::MediaType, tsc_config};

use deno_core::{error::AnyError, resolve_url_or_path, serde_json, ModuleSpecifier};
use std::{
    error::Error,
    fmt,
    rc::Rc,
...
use swc_common::{
    ...
    FileName, Globals, Loc, SourceFile, SourceMap, Span,
};
use swc_ecmascript::{
    ast::{Module, Program},
    codegen::{text_writer::JsWriter, Node},
    dep_graph::{analyze_dependencies, DependencyDescriptor},
    parser::{lexer::Lexer, EsConfig, JscTarget, StringInput, Syntax, TsConfig},
    transforms::{fixer, helpers, hygiene, pass::Optional, proposals, react, typescript},
    visit::FoldWith,
};
...
impl From<tsc_config::TsConfig> for EmitOptions {
    fn from(config: tsc_config::TsConfig) -> Self {
        let options: tsc_config::EmitConfigOptions = serde_json::from_value(config.0).unwrap();
...
fn strip_config_from_emit_options(options: &EmitOptions) -> typescript::strip::Config {
    let mut config = typescript::strip::Config::default();
    config.import_not_used_as_values = match options.imports_not_used_as_values {
        ImportsNotUsedAsValues::Remove => typescript::strip::ImportNotUsedAsValues::Remove,
        ImportsNotUsedAsValues::Preserve => typescript::strip::ImportNotUsedAsValues::Preserve,
        // `Error` only affects the type-checking stage. Fall back to `Remove` here.
        ImportsNotUsedAsValues::Error => typescript::strip::ImportNotUsedAsValues::Remove,
    };
    config
}
...
impl ParsedModule {
    ...
    pub fn transpile(self, options: &EmitOptions) -> Result<(String, Option<String>), AnyError> {
        ...
        let mut passes = chain!(
            ...
            typescript::strip::strip_with_config(strip_config_from_emit_options(options)),
...
pub fn parse_with_source_map(
    ...
    let source_file =
        source_map.new_source_file(FileName::Custom(specifier.to_string()), source.to_string());
...
pub fn transpile_module(
    ...
    let parsed_module = parse_with_source_map(filename, src, media_type, cm.clone())?;
    ...
    let mut passes = chain!(
        ...
        helpers::inject_helpers(),
        typescript::strip::strip_with_config(strip_config_from_emit_options(emit_options)),
...
mod tests {
    ...
    "#;
        let parsed_module = parse(specifier.as_str(), source, &MediaType::JavaScript)
    ...
    fn test_transpile() {
        let specifier =
            resolve_url_or_path("https://deno.land/x/mod.ts").expect("could not resolve specifier");
        let source = r#"
    enum D {
      A,
      B,
    ...
    "#;
        let module =
            parse(specifier.as_str(), source, &MediaType::TSX).expect("could not parse module");
        let (code, _) = module
            .transpile(&EmitOptions::default())
            .expect("could not strip types");
