...
use crate::{
    ...
    module_graph::{BundleType, EmitOptions, GraphBuilder},
    program_state::ProgramState,
    specifier_handler::{FetchHandler, MemoryHandler, SpecifierHandler},
};

use deno_core::{
    error::{generic_error, type_error, AnyError, Context},
    resolve_url_or_path, serde_json,
...
async fn op_emit(
    ...
    let handler: Arc<Mutex<dyn SpecifierHandler>> = if let Some(sources) = args.sources {
    ...
    let maybe_import_map = if let Some(import_map_str) = args.import_map_path {
        ...
        return Err(generic_error(
            "An importMap was specified, but no importMapPath was provided, which is required.",
        ));
    } else {
        None
    };
