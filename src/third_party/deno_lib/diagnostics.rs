...
use deno_core::serde::{Deserialize, Deserializer, Serialize, Serializer};
use regex::Regex;
...
lazy_static! {
    static ref MSG_MISSING_PROPERTY_DENO: Regex =
        Regex::new(r#"Property '([^']+)' does not exist on type 'typeof Deno'"#).unwrap();
    static ref MSG_SUGGESTION: Regex = Regex::new(r#" Did you mean '([^']+)'\?"#).unwrap();
}
...
mod tests {
    ...
    fn test_de_diagnostics() {
        ...
        let diagnostics: Diagnostics = serde_json::from_value(value).expect("cannot deserialize");
