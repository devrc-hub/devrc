...
use clap::{App, AppSettings, Arg, ArgMatches, ArgSettings, SubCommand};
use deno_core::{
...
fn repl_subcommand<'a, 'b>() -> App<'a, 'b> {
    runtime_args(SubCommand::with_name("repl"), false, true).about("Read Eval Print Loop")
}
...
fn inspect_args<'a, 'b>(app: App<'a, 'b>) -> App<'a, 'b> {
    ...
    .arg(
        Arg::with_name("inspect-brk")
            .long("inspect-brk")
            .value_name("HOST:PORT")
            .help("Activate inspector on host:port and break at start of user script")
            .min_values(0)
            .max_values(1)
            .require_equals(true)
            .takes_value(true)
...
fn reload_arg_parse(flags: &mut Flags, matches: &ArgMatches) {
    if let Some(cache_bl) = matches.values_of("reload") {
        let raw_cache_blocklist: Vec<String> = cache_bl.map(ToString::to_string).collect();
...
fn permission_args_parse(flags: &mut Flags, matches: &clap::ArgMatches) {
    ...
    if let Some(net_wl) = matches.values_of("allow-net") {
        let net_allowlist: Vec<String> =
            crate::flags_allow_net::parse(net_wl.map(ToString::to_string).collect()).unwrap();
...
mod tests {
    ...
    #[test]
    fn run_watch() {
        let r = flags_from_vec(svec!["deno", "run", "--unstable", "--watch", "script.ts"]);
        let flags = r.unwrap();
        assert_eq!(
            flags,
            Flags {
    ...
    fn run_reload_allow_write() {
        let r = flags_from_vec(svec!["deno", "run", "-r", "--allow-write", "script.ts"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                reload: true,
        ...
        assert_eq!(
            ...
            Flags {
                subcommand: DenoSubcommand::Fmt {
                    ignore: vec![],
                    check: false,
                    files: vec![PathBuf::from("script_1.ts"), PathBuf::from("script_2.ts")],
                    ext: "ts".to_string()
                },
                ..Flags::default()
            }
    ...
    fn lint() {
        ...
        assert_eq!(
            ...
            Flags {
                subcommand: DenoSubcommand::Lint {
                    ...
                    rules: false,
                    json: false,
                    ignore: vec![PathBuf::from("script_1.ts"), PathBuf::from("script_2.ts")],
                },
                unstable: true,
                ..Flags::default()
            }
        ...
        let r = flags_from_vec(svec!["deno", "lint", "--unstable", "--json", "script_1.ts"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Lint {
    ...
    #[test]
    fn tsconfig() {
        let r = flags_from_vec(svec!["deno", "run", "-c", "tsconfig.json", "script.ts"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Run {
    ...
    #[test]
    fn eval_typescript() {
        let r = flags_from_vec(svec!["deno", "eval", "-T", "'console.log(\"hello\")'"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Eval {
    ...
    fn allow_net_allowlist() {
        let r = flags_from_vec(svec!["deno", "run", "--allow-net=127.0.0.1", "script.ts"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Run {
    ...
    #[test]
    fn bundle_nocheck() {
        let r = flags_from_vec(svec!["deno", "bundle", "--no-check", "script.ts"]).unwrap();
        assert_eq!(
            r,
            Flags {
                subcommand: DenoSubcommand::Bundle {
    ...
    #[test]
    fn log_level() {
        let r = flags_from_vec(svec!["deno", "run", "--log-level=debug", "script.ts"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Run {
    ...
    #[test]
    fn run_with_cafile() {
        let r = flags_from_vec(svec!["deno", "run", "--cert", "example.crt", "script.ts"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Run {
    ...
    fn doc() {
        ...
        let r = flags_from_vec(svec!["deno", "doc", "--private", "path/to/module.js"]);
        assert_eq!(
            r.unwrap(),
            Flags {
                subcommand: DenoSubcommand::Doc {
