...
pub fn apply_source_map<G: SourceMapGetter>(js_error: &JsError, getter: Arc<G>) -> JsError {
    // Note that js_error.frames has already been source mapped in
    // prepareStackTrace().
    let mut mappings_map: CachedMaps = HashMap::new();

    let (script_resource_name, line_number, start_column, source_line) = get_maybe_orig_position(
        ...
        .map_or(true, |s| s.starts_with("deno:"))
    {
        for frame in &js_error.frames {
            if let (Some(file_name), Some(line_number_)) = (&frame.file_name, frame.line_number) {
                if !file_name.starts_with("deno:") {
                    ...
                    source_line = getter.get_source_line(file_name, (line_number_ - 1) as usize);
                    break;
                }
            }
        }
...
pub fn get_orig_position<G: SourceMapGetter>(
    ...
    let (file_name, line_number, column_number, source_line) = match maybe_source_map {
        None => default_pos,
        Some(source_map) => {
            match source_map.lookup_token(line_number as u32, column_number as u32) {
                ...
                Some(token) => match token.get_source() {
                    ...
                    Some(source_file_name) => {
                        ...
                        let source_line = if let Some(source_view) = token.get_source_view() {
                            source_view
    ...
    let source_line =
        source_line.or_else(|| getter.get_source_line(&file_name, line_number as usize));
...
fn parse_map_string<G: SourceMapGetter>(file_name: &str, getter: Arc<G>) -> Option<SourceMap> {
    getter
