...
use crate::{
    ...
    http_util::{create_http_client, fetch_once, FetchOnceArgs, FetchOnceResult},
...
fn get_source_from_data_url(
    ...
    let media_type_part =
        percent_encoding::percent_decode_str(parts.next().unwrap()).decode_utf8()?;
...
pub fn map_content_type(
    ...
    if let Some(content_type) = maybe_content_type {
        ...
        let media_type = match content_type.trim().to_lowercase().as_ref() {
            ...
            | "video/mp2t"
            | "application/x-typescript" => map_js_like_extension(specifier, MediaType::TypeScript),
            "application/javascript"
            | "text/javascript"
            | "application/ecmascript"
            | "text/ecmascript"
            | "application/x-javascript"
            | "application/node" => map_js_like_extension(specifier, MediaType::JavaScript),
...
fn map_js_like_extension(specifier: &ModuleSpecifier, default: MediaType) -> MediaType {
...
impl FileFetcher {
    ...
    fn fetch_data_url(&self, specifier: &ModuleSpecifier) -> Result<File, AnyError> {
        ...
        let (source, media_type, content_type) = get_source_from_data_url(specifier)?;
        let local = self
            .http_cache
            .get_cache_filename(specifier)
            .ok_or_else(|| generic_error("Cannot convert specifier to cached filename."))?;
...
mod tests {
    use super::*;
    use deno_core::{error::get_custom_error_class, resolve_url, resolve_url_or_path};
    use std::rc::Rc;
    ...
    fn setup(
        ...
        let temp_dir = maybe_temp_dir
            .unwrap_or_else(|| Rc::new(TempDir::new().expect("failed to create temp directory")));
        let location = temp_dir.path().join("deps");
        let file_fetcher = FileFetcher::new(HttpCache::new(&location), cache_setting, true, None)
            .expect("setup failed");
    ...
    async fn test_fetch_remote(specifier: &ModuleSpecifier) -> (File, HashMap<String, String>) {
    ...
    async fn test_fetch_remote_encoded(fixture: &str, charset: &str, expected: &str) {
        let url_str = format!("http://127.0.0.1:4545/cli/tests/encoding/{}", fixture);
    ...
    fn test_get_source_from_data_url() {
      ...
      ("data:text/tsx;base64,ZXhwb3J0IGRlZmF1bHQgZnVuY3Rpb24oKSB7CiAgcmV0dXJuIDxkaXY+SGVsbG8gRGVubyE8L2Rpdj4KfQo=", true, MediaType::TSX, "text/tsx;base64", "export default function() {\n  return <div>Hello Deno!</div>\n}\n"),
        ...
        for (url_str, expected_ok, expected_media_type, expected_media_type_str, expected) in
            fixtures
        {
            let specifier = resolve_url(url_str).unwrap();
            ...
            if expected_ok {
                let (actual, actual_media_type, actual_media_type_str) = actual.unwrap();
    ...
    async fn test_insert_cached() {
        ...
        let specifier = resolve_url_or_path(local.as_os_str().to_str().unwrap()).unwrap();
        let file = File {
            local,
            maybe_types: None,
    ...
    async fn test_get_source() {
        ...
        let specifier =
            resolve_url("http://localhost:4548/cli/tests/subdir/redirects/redirect1.js").unwrap();
        ...
        assert_eq!(
            file.specifier,
            resolve_url("http://localhost:4545/cli/tests/subdir/redirects/redirect1.js").unwrap()
        );
    }
    ...
    async fn test_fetch_complex() {
        ...
        let specifier =
            resolve_url_or_path("http://localhost:4545/cli/tests/subdir/mod2.ts").unwrap();
    ...
    async fn test_fetch_uses_cache() {
        ...
        let file_fetcher_01 =
            FileFetcher::new(HttpCache::new(&location), CacheSetting::Use, true, None)
                .expect("could not create file fetcher");
        let specifier =
            resolve_url("http://localhost:4545/cli/tests/subdir/mismatch_ext.ts").unwrap();
        ...
        let metadata_filename = crate::http_cache::Metadata::filename(&cache_filename);
        let metadata_file =
    ...
    async fn test_fetch_redirected() {
        ...
        let (file_fetcher, _) = setup(CacheSetting::Use, None);
        let specifier =
            resolve_url("http://localhost:4546/cli/tests/subdir/redirects/redirect1.js").unwrap();
    ...
    async fn test_fetch_uses_cache_with_redirects() {
        ...
        let file_fetcher_01 =
            FileFetcher::new(HttpCache::new(&location), CacheSetting::Use, true, None)
                .expect("could not create file fetcher");
        let specifier =
            resolve_url("http://localhost:4548/cli/tests/subdir/mismatch_ext.ts").unwrap();
        let redirected_specifier =
            resolve_url("http://localhost:4546/cli/tests/subdir/mismatch_ext.ts").unwrap();
        ...
        let metadata_filename = crate::http_cache::Metadata::filename(&redirected_cache_filename);
        let metadata_file =
            fs::File::open(metadata_filename).expect("could not open metadata file");
        let metadata_file_metadata = metadata_file.metadata().unwrap();
        let metadata_file_modified_01 = metadata_file_metadata.modified().unwrap();
    ...
    async fn test_fetch_same_host_redirect() {
        ...
        let (file_fetcher, _) = setup(CacheSetting::Use, None);
        let specifier =
            resolve_url("http://localhost:4550/REDIRECT/cli/tests/subdir/redirects/redirect1.js")
                .unwrap();
        let cached_filename = file_fetcher
            .http_cache
            .get_cache_filename(&specifier)
            .unwrap();
        let redirected_specifier =
            resolve_url("http://localhost:4550/cli/tests/subdir/redirects/redirect1.js").unwrap();
        let redirected_cached_filename = file_fetcher
            .http_cache
            .get_cache_filename(&redirected_specifier)
            .unwrap();
    ...
    async fn test_fetch_no_remote() {
        ...
        let location = temp_dir.path().join("deps");
        let file_fetcher =
            FileFetcher::new(HttpCache::new(&location), CacheSetting::Use, false, None)
                .expect("could not create file fetcher");
        let specifier = resolve_url("http://localhost:4545/cli/tests/002_hello.ts").unwrap();
    ...
    async fn test_fetch_cache_only() {
        ...
        let location = temp_dir.join("deps");
        let file_fetcher_01 =
            FileFetcher::new(HttpCache::new(&location), CacheSetting::Only, true, None)
                .expect("could not create file fetcher");
        let file_fetcher_02 =
            FileFetcher::new(HttpCache::new(&location), CacheSetting::Use, true, None)
                .expect("could not create file fetcher");
    ...
    async fn test_fetch_local_bypasses_file_cache() {
        ...
        let specifier = resolve_url_or_path(&fixture_path.to_string_lossy()).unwrap();
        ...
        fs::write(fixture_path, r#"console.log("goodbye deno");"#).expect("could not write file");
        let result = file_fetcher
            .fetch(&specifier, &Permissions::allow_all())
            .await;
        assert!(result.is_ok());
    ...
    async fn test_fetch_local_utf_16be() {
        let expected =
            String::from_utf8(b"\xEF\xBB\xBFconsole.log(\"Hello World\");\x0A".to_vec()).unwrap();
    ...
    async fn test_fetch_remote_with_types() {
        let specifier = resolve_url_or_path("http://127.0.0.1:4545/xTypeScriptTypes.js").unwrap();
    ...
    async fn test_fetch_remote_utf16_le() {
        let expected =
            std::str::from_utf8(b"\xEF\xBB\xBFconsole.log(\"Hello World\");\x0A").unwrap();
