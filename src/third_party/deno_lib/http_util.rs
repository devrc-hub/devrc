...
use deno_runtime::deno_fetch::{
    reqwest,
    reqwest::{
        header::{HeaderMap, HeaderValue, AUTHORIZATION, IF_NONE_MATCH, LOCATION, USER_AGENT},
        redirect::Policy,
        Client, StatusCode,
    },
};
...
pub async fn fetch_once(args: FetchOnceArgs) -> Result<FetchOnceResult, AnyError> {
    ...
    if let Some(auth_token) = args.maybe_auth_token {
        let authorization_val = HeaderValue::from_str(&auth_token.to_string()).unwrap();
...
mod tests {
    ...
    async fn test_fetch_gzip() {
        ...
        // Relies on external http server. See target/debug/test_server
        let url =
            Url::parse("http://127.0.0.1:4545/cli/tests/053_import_compression/gziped").unwrap();
        let client = create_test_client(None);
        let result = fetch_once(FetchOnceArgs {
            client,
            url,
    ...
    async fn test_fetch_brotli() {
        ...
        // Relies on external http server. See target/debug/test_server
        let url =
            Url::parse("http://127.0.0.1:4545/cli/tests/053_import_compression/brotli").unwrap();
        let client = create_test_client(None);
        let result = fetch_once(FetchOnceArgs {
            client,
            url,
    ...
    async fn test_fetch_with_cafile_gzip() {
        ...
        // Relies on external http server. See target/debug/test_server
        let url =
            Url::parse("https://localhost:5545/cli/tests/053_import_compression/gziped").unwrap();
        let client = create_http_client(
            version::get_user_agent(),
            Some(
                read(
    ...
    async fn test_fetch_with_cafile_brotli() {
        ...
        // Relies on external http server. See target/debug/test_server
        let url =
            Url::parse("https://localhost:5545/cli/tests/053_import_compression/brotli").unwrap();
        let client = create_http_client(
            version::get_user_agent(),
            Some(
                read(
