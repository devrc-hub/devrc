...
use crate::{
    ...
    module_graph::{Graph, Stats},
    tsc_config::TsConfig,
};

use deno_core::{
    error::{anyhow, bail, AnyError, Context},
    json_op_sync, resolve_url_or_path,
    serde::Deserialize,
    serde_json,
    serde_json::{json, Value},
    JsRuntime, ModuleSpecifier, OpFn, RuntimeOptions, Snapshot,
};
use std::{
    collections::HashMap,
    path::PathBuf,
...
fn get_maybe_hash(maybe_source: &Option<String>, hash_data: &[Vec<u8>]) -> Option<String> {
...
fn hash_data_url(specifier: &ModuleSpecifier, media_type: &MediaType) -> String {
    assert_eq!(
        specifier.scheme(),
        "data",
...
fn emit(state: &mut State, args: Value) -> Result<Value, AnyError> {
    let v: EmitArgs =
        serde_json::from_value(args).context("Invalid request from JavaScript for \"op_emit\".")?;
...
fn load(state: &mut State, args: Value) -> Result<Value, AnyError> {
    let v: LoadArgs =
        serde_json::from_value(args).context("Invalid request from JavaScript for \"op_load\".")?;
    ...
    // handle the request for that module here.
        ...
        let specifier = if let Some(data_specifier) = state.data_url_map.get(&v.specifier) {
            data_specifier.clone()
...
fn resolve(state: &mut State, args: Value) -> Result<Value, AnyError> {
    ...
    let referrer = if let Some(data_specifier) = state.data_url_map.get(&v.base) {
        ...
        resolve_url_or_path(&v.base)
            .context("Error converting a string module specifier for \"op_resolve\".")?
    };
        ...
        if specifier.starts_with("asset:///") {
            ...
            match graph.resolve(specifier, &referrer, true) {
                Ok(resolved_specifier) => {
                    ...
                    resolved.push((resolved_specifier_str, media_type.as_ts_extension().into()));
                }
...
fn respond(state: &mut State, args: Value) -> Result<Value, AnyError> {
    let v: RespondArgs =
        serde_json::from_value(args).context("Error converting the result for \"op_respond\".")?;
...
mod tests {
    ...
    use crate::{
        ...
        module_graph::{tests::MockSpecifierHandler, GraphBuilder},
        tsc_config::TsConfig,
    };
    ...
    async fn setup(
        ...
        let specifier =
            maybe_specifier.unwrap_or_else(|| resolve_url_or_path("file:///main.ts").unwrap());
    ...
    async fn test_exec(specifier: &ModuleSpecifier) -> Result<Response, AnyError> {
    ...
    async fn test_create_hash() {
        ...
        let actual = create_hash(&mut state, json!({ "data": "some sort of content" }))
    ...
    fn test_hash_data_url() {
        let specifier =
            deno_core::resolve_url("data:application/javascript,console.log(\"Hello%20Deno\");")
                .unwrap();
        assert_eq!(
            hash_data_url(&specifier, &MediaType::JavaScript),
    ...
    async fn test_emit() {
        ...
        assert_eq!(
            ...
            EmittedFile {
                ...
                maybe_specifiers: Some(vec![resolve_url_or_path("file:///some/file.ts").unwrap()]),
    ...
    async fn test_load_asset() {
        ...
        .await;
        let value = load(&mut state, json!({ "specifier": "asset:///lib.dom.d.ts" }))
            .expect("should have invoked op");
        let actual: LoadResponse = serde_json::from_value(value).expect("failed to deserialize");
    ...
    async fn test_load_tsbuildinfo() {
        ...
        .await;
        let actual = load(&mut state, json!({ "specifier": "deno:///.tsbuildinfo"}))
            .expect("should have invoked op");
        assert_eq!(
            actual,
            json!({
        ...
        assert_eq!(
            ...
            Some(RespondArgs {
                diagnostics: Diagnostics::new(vec![Diagnostic {
                    ...
                    message_text: Some("Unknown compiler option \'invalid\'.".to_string()),
                    message_chain: None,
                    source: None,
                    source_line: None,
                    file_name: None,
