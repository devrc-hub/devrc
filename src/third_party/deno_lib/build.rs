...
use deno_runtime::{deno_crypto, deno_fetch, deno_web, deno_webgpu, deno_websocket};
use regex::Regex;
use std::{
    collections::HashMap,
    env,
...
fn create_compiler_snapshot(snapshot_path: &Path, files: Vec<PathBuf>, cwd: &Path) {
...
fn ts_version() -> String {
    std::fs::read_to_string("tsc/00_typescript.js")
        ...
        .expect("Failed to find the pattern `ts.version = ` in typescript source code")
        .chars()
