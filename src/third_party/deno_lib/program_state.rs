...
use crate::{
    deno_dir,
    file_fetcher::{CacheSetting, FileFetcher},
    flags, http_cache,
    import_map::ImportMap,
    lockfile::Lockfile,
    module_graph::{CheckOptions, GraphBuilder, TranspileOptions, TypeLib},
    source_maps::SourceMapGetter,
    specifier_handler::FetchHandler,
    version,
};
use deno_runtime::{inspector::InspectorServer, permissions::Permissions};

use deno_core::{
    error::{anyhow, get_custom_error_class, AnyError, Context},
    resolve_url,
    url::Url,
...
pub struct ProgramState {
    ...
    pub modules: Arc<Mutex<HashMap<ModuleSpecifier, Result<ModuleSource, AnyError>>>>,
...
impl ProgramState {
    pub async fn build(flags: flags::Flags) -> Result<Arc<Self>, AnyError> {
        ...
        let file_fetcher =
            FileFetcher::new(http_cache, cache_usage, !flags.no_remote, ca_data.clone())?;
        ...
        let maybe_import_map: Option<ImportMap> = match flags.import_map_path.as_ref() {
            ...
            Some(import_map_url) => {
                ...
                let file = file_fetcher
                    ...
                    .await?;
                let import_map = ImportMap::from_json(import_map_specifier.as_str(), &file.source)?;
                Some(import_map)
            }
        };
    ...
    pub async fn prepare_module_load(
        ...
        let handler = Arc::new(Mutex::new(FetchHandler::new(self, runtime_permissions)?));
        let mut builder = GraphBuilder::new(handler, maybe_import_map, self.lockfile.clone());
...
fn source_map_from_code(code: String) -> Option<Vec<u8>> {
    ...
    if let Some(last_line) = lines.last() {
        if last_line.starts_with("//# sourceMappingURL=data:application/json;base64,") {
            let input =
                last_line.trim_start_matches("//# sourceMappingURL=data:application/json;base64,");
            let decoded_map =
