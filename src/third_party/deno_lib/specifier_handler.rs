...
use crate::{
    ast::Location, deno_dir::DenoDir, disk_cache::DiskCache, file_fetcher::FileFetcher,
    media_type::MediaType, program_state::ProgramState,
};
...
use deno_core::{
    ...
    serde_json, ModuleSpecifier,
};
use std::{collections::HashMap, env, fmt, path::PathBuf, pin::Pin, sync::Arc};

pub type DependencyMap = HashMap<String, Dependency>;
pub type FetchFuture = Pin<
    Box<(dyn Future<Output = Result<CachedModule, (ModuleSpecifier, AnyError)>> + 'static + Send)>,
>;
...
pub trait SpecifierHandler: Sync + Send {
    ...
    fn get_tsbuildinfo(&self, specifier: &ModuleSpecifier) -> Result<Option<String>, AnyError>;
    ...
    fn set_types(&mut self, specifier: &ModuleSpecifier, types: String) -> Result<(), AnyError>;
    ...
    fn set_version(&mut self, specifier: &ModuleSpecifier, version: String)
        -> Result<(), AnyError>;
}
...
impl SpecifierHandler for FetchHandler {
        ...
        async move {
            ...
            let maybe_version = if let Some(filename) = filename {
                if let Ok(bytes) = disk_cache.get(&filename) {
                    if let Ok(compiled_file_metadata) = CompiledFileMetadata::from_bytes(&bytes) {
            ...
            let map_path = disk_cache.get_cache_filename_with_extension(&url, "js.map");
            ...
            if let Some(emit_path) = emit_path {
                if let Ok(code) = disk_cache.get(&emit_path) {
                    maybe_emit = Some(Emit::Cli((String::from_utf8(code).unwrap(), maybe_map)));
                    maybe_emit_path = Some((disk_cache.location.join(emit_path), maybe_map_path));
                }
            };
    ...
    fn set_types(&mut self, _specifier: &ModuleSpecifier, _types: String) -> Result<(), AnyError> {
...
impl SpecifierHandler for MemoryHandler {
    ...
    fn get_tsbuildinfo(&self, _specifier: &ModuleSpecifier) -> Result<Option<String>, AnyError> {
        Ok(None)
    }

    fn set_cache(&mut self, _specifier: &ModuleSpecifier, _emit: &Emit) -> Result<(), AnyError> {
        Ok(())
    }
...
pub mod tests {
    use super::*;
    use crate::{file_fetcher::CacheSetting, http_cache::HttpCache};
    ...
    fn setup() -> (TempDir, FetchHandler) {
        ...
        let deno_dir = DenoDir::new(Some(temp_dir.path().to_path_buf())).expect("could not setup");
    ...
    async fn test_fetch_handler_is_remote() {
        ...
        let specifier =
            resolve_url_or_path("http://localhost:4545/cli/tests/subdir/mod2.ts").unwrap();
        let cached_module: CachedModule = file_fetcher.fetch(specifier, None, false).await.unwrap();
        assert_eq!(cached_module.is_remote, true);
        let c = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").unwrap());
        let specifier =
            resolve_url_or_path(c.join("tests/subdir/mod1.ts").as_os_str().to_str().unwrap())
                .unwrap();
        let cached_module: CachedModule = file_fetcher.fetch(specifier, None, false).await.unwrap();
    ...
    "#;
        ...
        let specifier = resolve_url_or_path("https://deno.land/x/missing.ts").unwrap();
        handler
            .fetch(specifier.clone(), None, false)
            .await
