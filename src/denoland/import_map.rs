...
const SPECIAL_PROTOCOLS: &[&str] = &["ftp", "file", "http", "https", "ws", "wss"];
...
impl ImportMap {
    pub fn from_json(base_url: &str, json_string: &str) -> Result<Self, ImportMapError> {
        ...
        for key in keys {
            diagnostics.push(format!(
                "Invalid top-level key \"{}\". Only \"imports\" and \"scopes\" can be present.",
                key
            ));
        }
    ...
    fn parse_specifier_map(
        ...
        for (specifier_key, value) in json_map.iter() {
            let normalized_specifier_key =
                match ImportMap::normalize_specifier_key(specifier_key, base_url, diagnostics) {
                    Some(s) => s,
                    None => continue,
                };
            ...
            let address_url = match ImportMap::try_url_like_specifier(&potential_address, base_url)
            {
                Some(url) => url,
                None => {
                    diagnostics.push(format!(
    ...
    fn parse_scope_map(
        ...
        for (scope_prefix, potential_specifier_map) in scope_map.iter() {
            ...
            let norm_map =
                ImportMap::parse_specifier_map(potential_specifier_map, base_url, diagnostics);
    ...
    fn resolve_scopes_match(
        ...
        if let Some(scope_imports) = scopes.get(referrer) {
            let scope_match =
                ImportMap::resolve_imports_match(scope_imports, normalized_specifier, as_url)?;
    ...
    pub fn resolve(&self, specifier: &str, referrer: &str) -> Result<Option<Url>, ImportMapError> {
        let as_url: Option<Url> = ImportMap::try_url_like_specifier(specifier, referrer);
...
mod tests {
    ...
    fn load_import_map_wpt_tests() -> Vec<String> {
        ...
        let import_map_wpt_path = repo_root.join("test_util/wpt/import-maps/data-driven/resources");
    ...
    fn parse_import_map_tests(test_str: &str) -> Vec<ImportMapTestCase> {
        ...
        fn parse_test_object(
            ...
            let maybe_import_map_base_url = if let Some(base_url) = test_obj.get("importMapBaseURL")
            {
        ...
        for test in tests {
            match &test.kind {
                TestKind::Resolution {
                    ...
                    let import_map =
                        ImportMap::from_json(&test.import_map_base_url, &test.import_map).unwrap();
                    let maybe_resolved = import_map.resolve(&given_specifier, &base_url).ok().map(
                        |maybe_resolved| {
